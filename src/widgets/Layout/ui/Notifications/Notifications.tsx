...
import { getNotifications, setNotificationReaded } from "http/siteApi";
...
export const Notifications: React.FC<NotificationsProps> = (props) => {
  ...
  const handleClose = (id: number) => {
    const updatedNotifications = notifications.filter((n: Notificaion) => n.id !== id);
    setNotifications(updatedNotifications);
    setNotificationReaded(id);
  };

  const __load_notification_async = async() => {
    try {
    ...
    <div className={clsx(classes.notifications, className)}>
      ...
      {!!notifications.length && (
        <div className={classes.list}>
          {notifications.map(({ text, link, text_link, id } : Notificaion) => (
            <div className={classes.item} key={text}>
              <div className={classes.close} onClick={() => handleClose(id)}>
                <CloseIcon />
              </div>
              <div className={classes.text}>{text}</div>
              { link !== null && link !== "" && (
                <a className={classes.link} href={link} target="_blank">
                  { (text_link == "" || text_link == null) ? "Подробнее" : text_link } 
                </a>
              )}
            </div>
          ))}
